...
<table class="messenger-list-item @if($user->id == $id && $id != "0") m-list-active @endif" data-contact="{{ $user->id }}">
    <tr data-action="0">
        ...
        style="background-image: url('{{ asset('/images/public/users-avatar/'.$user->avatar) }}');">
        </div>
    </td>
    {{-- center side --}}
    <td>
        <p data-id="{{ $type.'_'.$user->id }}">
            ...
            <span class="badge badge-warning" style="text-transform: capitalize;
            font-size: 10px;color:#000;
            margin-bottom: 5px;">{{ $user->type }}</span> <br>
        <span>
