    ...
    if (messenger != 0) {
        ...
        $.ajax({
            ...
            success: (res) => {
                // avatar photo
                console.log(res);
                $('.messenger-infoView').find('.avatar').css('background-image', 'url("/images/public/users-avatar/' + res.fetch.avatar + '")');
                $('.header-avatar').css('background-image', 'url("/images/public/users-avatar/' + res.fetch.avatar + '")');
                ...
                messageInput.focus();
                // update info in view
                $('.messenger-infoView .info-name').html(res.fetch.username);
                $('.m-header-messaging .user-name').html(res.fetch.username);
                // Star status
                res.favorite > 0 ?
